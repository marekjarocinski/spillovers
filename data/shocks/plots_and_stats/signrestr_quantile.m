function [U, C, a, a_range] = signrestr_quantile(M, w)
...
% Find the range of rotation angles that satisfy the sign restrictions.
% By default returns the median shocks.
% INPUTS:
% M - T by 2 matrix
...
% a - rotation angle
% a_range - admissible range of rotation angles
%
% SEE:
...
Q = Q*S; R = S*R;
if R(1,2)>0
    a_range = [atan(R(1,2)/R(2,2)), pi/2];
    %a = (1-w)*atan(R(1,2)/R(2,2)) + w*pi/2;
else
    a_range = [0 atan(-R(2,2)/R(1,2))];
    %a = (1-w)*0 + w*atan(-R(2,2)/R(1,2));
end
a = (1-w)*a_range(1) + w*a_range(2);
