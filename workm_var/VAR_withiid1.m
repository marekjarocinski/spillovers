...
%     Sigma = chol(prior.S)'/(temp'*temp)*chol(prior.S);
    ...
    for draw = 1:nalldraws
        ...
        for n = 1:Ny
            % indices
            JJ = 1:(Nm+(n-1));
            kk = ((n-1)*K+1):(n*K); % ind of coefs of eq n in vecB
            % MM = 1:(n-1)*K; needed only when prior.Q has nonzero off-diagonal blocks
            % matrices
            % Sigmann1inv = 1/(Sigma(n,n)  - Sigma(n,JJ)/Sigma(JJ,JJ)*Sigma(JJ,n));
            Sigmann1inv = Csig(Nm+n,Nm+n)^-2;
            Qkk1inv = prior.Qinv(kk,kk); % assuming prior.Q is (block-)diagonal
            A = Qkk1inv + Sigmann1inv*(X'*X);
            % yst = Y(:,Nm+n) - (Y(:,JJ) - X*BB(:,JJ))/Sigma(JJ,JJ)*Sigma(JJ,Nm+n);
            yst = Y(:,Nm+n) - (Y(:,JJ) - X*BB(:,JJ))/Csig(JJ,JJ)'*Csig(Nm+n,JJ)';
            a = prior.QinvB_reshaped(:,n) + X'*yst*Sigmann1inv;
            C = chol(A);
            BB(:,Nm+n) = C \ (C'\a + randn(K,1)); % Chan (2015) formula
        end
